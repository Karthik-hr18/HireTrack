...
router.post('/', authenticate, requireVerifiedEmail, authorize('recruiter', 'admin'), scheduleInterview);
router.get('/mine', authenticate, authorize('recruiter', 'admin'), getAdminInterviews);

// Scorecard submission (Admins only)
router.post('/:id/scorecard', authenticate, requireVerifiedEmail, authorize('admin'), submitScorecard);
